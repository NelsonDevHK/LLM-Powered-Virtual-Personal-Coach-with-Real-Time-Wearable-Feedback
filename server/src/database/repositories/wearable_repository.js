...
class WearableRepository {
    ...
    async findRecentByUserId(userId, limit = 5) {
        ...
        const query = `
            SELECT data_id, user_id, heart_rate, exercise_type, set_count,
    ...
    async save(userId, data) {
        const query = `
            ...
            (user_id, heart_rate, exercise_type, set_count, sleep_duration, sleep_quality, rest_duration, recorded_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
        `;
        const values = [
            userId,
